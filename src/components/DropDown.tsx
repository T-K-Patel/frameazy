...
const DropDown = ({ items, value, onChange }: { items: string[], value: string, onChange: any }) => {
    const options = items.map((item) => { return { value: item, label: item } });
    ...
    return (
        <Select onChange={(e) => { onChange(e.value) }} styles={colourStyles} options={options} value={{ value, label: value }} />
    )
}
