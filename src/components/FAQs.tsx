...
const Accordion = ({ index, title, isOpen, data }: AccordionProps) => {
    return (
        <div className="my-4 border-b-2 border-b-[#f1f1f1]">
            <h2
                className="mb-4 w-full text-left transition duration-300 flex justify-between cursor-pointer"
                data-faq-id={index}
            >
                <span className="md:text-xl font-medium">{title}</span>
                {isOpen ? (
                    <span
...
const FAQ = () => {
    ...
    return (
        <div className="w-[89%] mx-auto mt-10 lg:mt-16">
